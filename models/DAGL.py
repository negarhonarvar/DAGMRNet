...
class CE(nn.Module):
    ...
    def forward(self, b):
        ...
        # print("soft thr shape :" , soft_thr.shape)
        # print("soft bias shape :" , soft_bias.shape)
        ...
        # print("Patch 28 size :" , patch_28.shape)
        # print("Patch 112 :" , patch_112.shape)
        y = []
        ...
        for xi, wi,pi,thr,bias in zip(patch_112_group_2, patch_28_group, patch_112_group,soft_thr,soft_bias):
            ...
            # print("scoremap shape" , b_s, l_s, h_s, w_s)
...
class CES(nn.Module):
    ...
    def forward(self, x):
        # Stage 1 (4-head)
        # print("c1's shapes : " , self.c1_1(x).shape, self.c1_2(x).shape)
        # print("x shape :" ,x.shape)
        # out = self.c1_c(torch.cat((self.c1_1(x), self.c1_2(x), self.c1_3(x), self.c1_4(x)), dim=1)) + x
         # Stage 1 (4-head)
        out = checkpoint.checkpoint(self.c1_c, self.c1_1(x)) + x
        out = checkpoint.checkpoint(self.RBS1, out)

        # Stage 2 (4-head)
        # out = checkpoint.checkpoint(self.c2_c, torch.cat((self.c2_1(out), self.c2_2(out), self.c2_3(out), self.c2_4(out)), dim=1)) + out
        # out = checkpoint.checkpoint(self.RBS2, out)
