...
class DAGMRNet(nn.Module):
    def __init__(
        self,
        in_chans: int = 2,
        out_chans: int = 2,
        n_feat0: int = 48, # it used to be 48
        feature_dim: List[int] = [72, 96, 120], # it used to be [72, 96, 120]
        prompt_dim: List[int] =[24, 48, 72] , # it used to be [24, 48, 72]
        len_prompt: List[int] = [5, 5, 5],
...
class PromptMRBlock(nn.Module):
    ...
    def __init__(self, model: nn.Module, num_adj_slices=1):
        """
        Args:
