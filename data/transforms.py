...
class DAGMRNetDataTransform:
    ...
    def __call__(
        ...
        seed = None if not self.use_seed else tuple(
            map(ord, fname))  # so in validation, the same fname (volume) will have the same acc
...
class FastmriKneeDAGMRNetDataTransform:
    ...
    def _pad_if_needed(self, image):
        ...
        return torch.nn.functional.pad(image.permute(0, 3, 1, 2), (pad_h_left, pad_h_right, pad_w_left, pad_w_right),
                                       'reflect').permute(0, 2, 3, 1)
