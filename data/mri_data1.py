...
# ye class misazim ke data ro ke mikhunim inja save konim
...
class CmrxReconSliceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if dataset_cache.get(root) is None or not use_dataset_cache:
            ...
            for fname in sorted(files): 
                with h5py.File(fname,'r+', swmr=False) as hf:
        ...
        if 'train' in str(root):  # hala ke hama ro ghati kardim betunim joda konim
            raw_samples_t1 = []
            raw_samples_t2 = []
            raw_samples_lax = []
            raw_samples_sax = []
            ...
            self.raw_sample = raw_samples_lax + raw_samples_sax + raw_samples_lvot
    ...
    # umadim slice haro avaz kardam t*s kardim , hala miaym hessab mikonim ke shomare slice adjacent to space feeli chi mishe
    ...
    def _get_frames_indices_mapping(self, dataslice, num_slices_in_volume, num_t_in_volume=None, isT2=False):
        ...
        if isT2: # only 3 nw in T2, so we repeat adjacent for 3 times
            ...
            ti_idx_list = [ (i+ti)%num_t_in_volume for i in range(-2,3)]  # vasati ro 3 bar tekrar mikonim
        output_list = []
    ...
    def __getitem__(self, i: int):
        ...
        with h5py.File(str(fname),'r') as hf:
            ...
            # har data ro be format matloobemoon misazim va data ro amade mikonim ke be model bedim
