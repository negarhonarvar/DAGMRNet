...
def count_parameters(model1 , model2):
    gnn_param = sum(p.numel() for p in model2.parameters()) if model2 is not None else 0
    print("gnn param = " , gnn_param)
    promptmr_param = sum(p.numel() for p in model1.parameters()) if model1 is not None else 0 
